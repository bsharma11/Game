...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        team = sqlite3.connect('fantasy.db')
        conn = team.cursor()
    ...
    def menu(self,action):
        ...
        if txt=='SAVE Team':
            ...
            for i in range(count):
                ...
                if i<count:
                    selected+=","
            self.saveteam(self.t7.text(),selected,self.used,self.points)
        ...
        if txt=='EVALUATE Team':
            ...
            if ok and team:
                s = "SELECT points from teams where name ='"+team+"'"
                cur = conn.execute(s)
                row = cur.fetchone()
                a = int(row[0])
                msg = "Points scored by your team: "+str(a)
                self.showdlg(msg)
                return
    ...
    def criteria(self,ctgr,item):
        ...
        sql="SELECT value, points from stats where player='"+item.text()+"'"
        cur=conn.execute(sql)
        row=cur.fetchone()
        self.avl-=int(row[0])
        self.used+=int(row[0])
    ...
    def saveteam(self,nm,ply,val,points):
        #print("rvrv")
        ...
        #print("frbfj")
        sql="INSERT INTO teams (name,players,value,points) VALUES ('"+nm+"','"+ply+"','"+str(val)+"','"+str(points)+"');"
        #print("f3f4")
        try:
            #print("bjr")
            cur=conn.execute(sql)
